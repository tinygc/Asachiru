...
### 最新の更新 (2025-12-02)
🎯 **包括的なドキュメント・説明文更新完了！**
1. **Google Play Store 掲載情報を全面更新** 📱
   - `store_assets/store_listing.md` (AsaChil): スマートフォン対応反映、全機能説明追加
   - `docs/feedwatch/PLAY_STORE_LISTING.md` (FeedWatch): AsaChil との違いを明確化
   - 実装仕様（Issue #106-#108）を反映、Device別操作方法を詳細に記載
2. **README.md の主要機能セクション更新** 📖
   - Issue #106-#108 の機能を明記（TTS表示修正、権限削除、待機時間表示）
   - State Machine パターン説明を追加
   - AsaChil vs FeedWatch の機能比較表を追加
   - UI/UX 改善（矢印表示、ポップアップ機能）を明記
...
4. **Android TV向け記事ナビゲーションUI改善** 🆕
   - メイン画面に上下の大きい矢印（▲▼）を追加して、リモコンの上下キーで直感的に記事送り・戻しできるようにしたよ！
   - 記事の先頭/末尾では矢印が非表示になるから、どこまで読んだか一目でわかる！
   - ViewBinding・状態管理（MainUiState, MainViewModel）もバッチリ修正済み！
   - TDDでテストも全件パス！E2EテストもOK👌
