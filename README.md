...
- **BGM音量**: 常時30%固定で再生（落ち着いた音量でリラックス効果を重視）
...
### 最近の更新 (2025-11-20)
1. **BGM音量の固定化**: BGM音量を常時30%に固定し、より落ち着いた音量でリラックス効果を重視
   - MusicPlayerのbaseVolumeを0.3f(30%)に変更
   - TtsManagerのダッキング処理を削除（すでに30%固定のため不要）
   - ニュース読み上げ中も音量変化なし

### 過去の更新 (2025-11-19)
1. **バックグラウンド移行時のBGM停止**: アプリをバックグラウンドに移動した際にBGMが鳴り続ける問題を修正
   ...
   - アプリライフサイクルに応じた適切なリソース管理により、アプリが終了しやすくなりました
...
3. **テキストサイズ最適化**: 実際のTV表示に最適化するため、すべてのテキストサイズをDP単位（SP）に変換し約50%縮小
   - カスタムビュー（時計/天気/ニュース/音楽）でTypedValue.applyDimension()を使用してSP→PX変換
   - 設定画面とスプラッシュ画面のテキストサイズも調整
4. **音楽再生情報の表示強化**: 再生中のMP3の情報をメタデータから取得して画面に表示
...
4. MusicPlayerをアプリ全域で共有するグローバルシングルトンへ変更（Visualizer連携のため）
5. Visualizer起動ロジックを`MainActivity`へ追加しAudioSessionId変化時に再初期化するよう改善（ログ付き）
6. RECORD_AUDIO権限要求と許可後即Visualizer再初期化処理追加 / onPauseでfinish()削除
7. Visualizer失敗時リトライ(最大3回)＋平滑化導入で過度なチラつきを低減 / Fallback時は独自アニメ表示
8. FFTモード実装（waveform/FFT切替可能）＆ dBスケール正規化で音楽同期精度向上
