...
- **実行状況**: ⚠️ **未実行**。本対応はAGPプラグインリポジトリへの
  ネットワークアクセスができないサンドボックス環境で行ったため、
  `gradlew testAsachiruDebugUnitTest --tests
  "com.tinygc.asachiru.domain.util.DeviceUtilsTest"`
  を実行してPASSを確認できていない。静的コードレビュー（実装とアサーション
  の突き合わせ）のみで正しさを確認した状態であり、マージ前に実機/CI環境で
  実行してこのセクションを実測結果で更新すること。
