...
#### 2-4. 記事詳細のレイアウト（縦画面）
```
Given: スマホ縦画面で記事詳細を表示している
When: タイトル/本文が長い記事を表示する
Then:
  - 表示順は「タイトル → 記事詳細 → QRコード」
  - QRコードと本文が重ならない
  - 省略が発生する場合は、重なる直前の行末が「…」になる
  - フッター（再タップで閉じる）がQRコードの上に表示される
```

**対応テスト**: Manual test (スマホ縦画面)

#### 2-5. 記事詳細のレイアウト（横画面）
```
Given: スマホ横画面で記事詳細を表示している
When: タイトル/本文が長い記事を表示する
Then:
  - 右側にQRコード領域が確保され、本文が重ならない
  - 本文が画面下端を越えて表示されない
  - フッター（再タップで閉じる）が画面下端付近に表示される
```

**対応テスト**: Manual test (スマホ横画面)
