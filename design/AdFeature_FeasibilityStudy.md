...
| 状態 | 説明 | 継続時間 | 広告表示の適性 | 実装状況 |
|------|------|----------|---------------|----------|
| `Idle` | 初期状態 | 瞬間的 | ❌ 不適 | - |
| `WaitingForStart` | アプリ起動後の待機 | 10秒広告 + 3秒待機 | ✅ 適 | ✅ 実装済み |
| `FetchingNews` | ニュース取得中 | 数秒 | ⚠️ 短い | - |
| `ArticleInterval` | 記事間インターバル | 5秒 (TTS ON時のみ) | ⚠️ UX悪化 | ❌ 不採用 |
| `SessionInterval` | セッション間待機 | 10秒広告 + 5分待機 | ✅ 最適 | ✅ 実装済み |

**TTS OFF時の注意点:**
- 現在、TTS OFFの場合は`ArticleInterval`をスキップして即座に次の記事に移動
- つまり、記事から記事への切り替えが連続的

### 2.2 推奨広告表示タイミング（2025年2月更新）

**実装済み戦略: 起動時 + 全記事終了時の2回表示**

アプリ起動時とセッション終了時の2箇所で広告を表示することで、ユーザ体験を損なわずに収益を最大化する戦略を採用しました。

#### タイミング1: `WaitingForStart` (起動時待機) - 実装済み ✅
- **継続時間**: 10秒広告 + 3秒待機
- **頻度**: アプリ起動ごとに1回
- **広告表示時間**: 10秒
- **メリット**: 
  ...
  - アプリ起動直後で印象に残りやすい
- **実装状況**: ✅ 実装済み
  - スマホ: AdMob インタースティシャル広告
  - Android TV: QRコード経由でスマホ版への誘導

#### タイミング2: `SessionInterval` (セッション間待機) - 実装済み ✅
- **継続時間**: 10秒広告 + 5分待機
...
#### 収益シミュレーション（DAU 100の場合）
- 1日あたり平均セッション数: 2回（起動時1回 + セッション終了1回）
- 1日あたり広告表示回数: 100 DAU × 2回 = 200回
- 月間広告表示回数: 200回 × 30日 = 6,000回
- 想定CPM: ¥700（AdMob平均）
- **月間推定収益**: 6,000 ÷ 1,000 × ¥700 = **¥4,200**

**検討したが不採用の選択肢:**

#### 優先度3: `ArticleInterval` (記事間インターバル - TTS ON時のみ)
- **継続時間**: 5秒
- **頻度**: TTS ON時、記事と記事の間（10記事なら9回）
- **不採用理由**: 
  - 記事読了の「chill」「リラックス」というコンセプトに反する
  - 頻繁な広告挿入でユーザ体験が大きく損なわれる
  - TTS OFF時は存在しないため、機能の一貫性に欠ける
  - 実装・メンテナンスコストに対する収益増が限定的
...
## 補足2: 収益試算 - 記事セッション終了後10秒広告（実装済み）

### シナリオ設定

**実装方法:**
- 全記事読了後 (AllArticlesCompleted)
- SessionIntervalの最初の10秒間に広告表示（スマホのみ）
- Android TVでは同タイミングでスマホ版QRコード誘導を表示
- 残り時間 (29分50秒) は広告なし待機
